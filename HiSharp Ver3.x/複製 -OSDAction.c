...
if(VlossFlag==0)
{
	PCT_PreSetForegroundColor(OSD_COL_BLU);
	PCT_ClearOSDPageX(WRPHX_PAGE2);
	PCT_SetOSDDrawPage(WRPHX_PAGE2);

	PCT_ShowVLoss();   //ryan@20150318
	PCT_EnShowOSDPage(WRPHX_PAGE2);
#if 0 //def TW2835_FUNC_DEFINE  //Pinchi 20150324 add
...
//	ifBitSet(SYSWINATTR[0],BIT1)	
   ...
   PCT_OSD_ActionPthX(SYSWINATTR); 
	   ...
	   PCT_ShowSpeakIcon(SYSWINATTR);//andy 20130619 remove Audio Icon
...
#else
	if (VlossFlag == 0)  //pinchi test
	{
	VlossFlag=1;

	PCT_PreSetForegroundColor(OSD_COL_RED255|OSD_BLINK);
#if 1 //def AWT_NO_SIGNAL_MSG  //Pinchi 20150323 add
	PCT_ShowVLoss();
#else
...
//add by ryan@20150305
#define VLS_LCT_XM0	(LOCATE_XM0-4)-6
#define VLS_LCT_XM1	(LOCATE_XM1-4)-6
#define VLS_LCT_XM2	(LOCATE_XM2-4)-6
#else
